#!/usr/bin/env python3
"""
Claw Bot v6 - EXECUTION ENABLED
Monitors + executes: liquidations, yield opportunities, on-chain actions
Wallet funded -> revenue starts
"""
...
def run_cycle(state):
    ...
    # Check wallet balance every 10 cycles
    if c % 10 == 1:
    ...
    report = {
        ...
        "stats":{"cycles":c,"total_alerts":len(state["alerts"]),"executions":len(state.get("executions",[]))}
    }
    (LOG_DIR / "latest.json").write_text(json.dumps(report, indent=2, default=str))
    return report

def main():
    log.info("Claw Bot v6 | EXECUTION ENABLED | Wallet-aware")
    log.info(f"Wallet: {WALLET}")
    log.info(f"Web3 connected: {w3.is_connected()}")
    log.info(f"Interval: {INTERVAL}s")

    state = load_state()
    log.info(f"Resuming from cycle {state['cycles']} | Revenue: {state.get('revenue_eth',0)} ETH")
